...
interface TimelineCardProps {
  ...
  onViewEvidence: (statement: StatementRecord) => void;
  isSelected: boolean;
}

export function TimelineCard({ statement, onViewEvidence, isSelected }: TimelineCardProps) {
  return (
    <article
    ...
    >
      <div className="px-4 pt-3.5 pb-3">
        <div className="flex flex-wrap items-center gap-1.5 mb-2.5">
          ...
          <div className="flex items-center gap-1.5 ml-auto text-[11px] text-stone-400">
            ...
            <span>{formatDate(statement.sourceDate)}</span>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-[11px] text-stone-400 mb-2.5">
          ...
          <span>
            ...
            {CONFIDENCE_LABELS[statement.confidenceScore] || "Medium"}
          </span>
        </div>
      ...
      <div className="flex items-center gap-1 px-4 py-2 border-t border-stone-100 bg-stone-50/40 rounded-b-lg">
        <button
          onClick={(e) => { e.stopPropagation(); onViewEvidence(statement); }}
