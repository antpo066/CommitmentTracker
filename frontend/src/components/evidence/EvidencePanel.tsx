...
import type { StatementRecord } from "@/lib/api";
import { formatDateLong } from "@/lib/utils";
import { CONFIDENCE_LABELS } from "@/lib/constants";
import { StatusBadge } from "@/components/ui/StatusBadge";
import { TypeBadge } from "@/components/ui/TypeBadge";
import { X, ExternalLink, Flag, MessageSquare, FileText, ShieldCheck } from "lucide-react";
    ...
    <div className="space-y-4">
      ...
      <div className="flex gap-4">
        <div className="flex-1">
          ...
          <p className="text-xs text-stone-700">{statement.sourceTitle}</p>
          {statement.sourceDocument && (
            <p className="text-[11px] text-stone-400 mt-0.5">
              via {statement.sourceDocument.sourceType.replace('_', ' ').toLowerCase()}
            </p>
          )}
          {statement.sourceUrl && (
        ...
        <div>
          <h4 className="text-[10px] font-semibold text-stone-400 uppercase tracking-wider mb-0.5">Date</h4>
          <p className="text-xs text-stone-700">{formatDateLong(statement.sourceDate)}</p>
        </div>
      </div>

      {statement.context && (
        <div>
          <h4 className="text-[10px] font-semibold text-stone-400 uppercase tracking-wider mb-1">Original Context</h4>
          <p className="text-xs text-stone-600 leading-relaxed">{statement.context}</p>
        </div>
      )}

      {statement.interpretation && (
        <div>
          <h4 className="text-[10px] font-semibold text-stone-400 uppercase tracking-wider mb-1">Interpretation</h4>
          <p className="text-xs text-stone-600 leading-relaxed">{statement.interpretation}</p>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 gap-2">
        {statement.impliedDeadline && (
          <div>
            <h4 className="text-[10px] font-semibold text-stone-400 uppercase tracking-wider mb-0.5">Deadline</h4>
            <p className="text-xs text-stone-700">{formatDateLong(statement.impliedDeadline)}</p>
          </div>
        )}
        {statement.measurableOutcome && (
          <div className={statement.impliedDeadline ? "" : "col-span-2"}>
            <h4 className="text-[10px] font-semibold text-stone-400 uppercase tracking-wider mb-0.5">Outcome</h4>
            <p className="text-xs text-stone-700">{statement.measurableOutcome}</p>
          </div>
        )}
        <div>
          <h4 className="text-[10px] font-semibold text-stone-400 uppercase tracking-wider mb-0.5">Confidence</h4>
          <div className="flex items-center gap-1">
            <div className="flex gap-px">
              {[1, 2, 3, 4, 5].map((i) => (
                <div key={i} className={`h-1 w-2.5 rounded-full ${i <= statement.confidenceScore ? "bg-stone-400" : "bg-stone-200"}`} />
              ))}
            </div>
            <span className="text-[11px] text-stone-500">{CONFIDENCE_LABELS[statement.confidenceScore]}</span>
          </div>
        </div>
      </div>
      ...
      {statement.evidence && statement.evidence.length > 0 && (
        <div>
          <h4 className="text-[10px] font-semibold text-stone-400 uppercase tracking-wider mb-2 flex items-center gap-1">
            <ShieldCheck className="h-3 w-3" />
            Evidence ({statement.evidence.length})
          </h4>
          <div className="space-y-1.5">
            {statement.evidence.map((ev) => (
              <div key={ev.id} className="rounded-md bg-stone-50 border border-stone-200 p-2.5 border-l-2 border-l-stone-300">
                <p className="text-xs text-stone-600 leading-relaxed">{ev.excerpt}</p>
                <div className="flex items-center gap-2 mt-1.5 text-[10px] text-stone-400">
                  {ev.sourceTitle && <span>{ev.sourceTitle}</span>}
      ...
      {statement.adminNotes && (
        <div className="rounded-md bg-stone-50 border border-stone-200 p-3">
          <h4 className="text-[10px] font-semibold text-stone-500 uppercase tracking-wider mb-1">Admin Notes</h4>
          <p className="text-xs text-stone-600 leading-relaxed">{statement.adminNotes}</p>
        </div>
      )}
        ...
        <div>
          ...
          <div className="space-y-1.5">
            {statement.notes.map((note) => (
              <div key={note.id} className="rounded border border-stone-100 bg-stone-50/50 p-2.5">
                <div className="flex items-center gap-1.5 mb-1">
                  <MessageSquare className="h-2.5 w-2.5 text-stone-400" />
                  <span className="text-[10px] font-medium text-stone-500 capitalize">{note.noteType.toLowerCase()}</span>
                  <span className="text-[10px] text-stone-400">· {note.author.name}</span>
                </div>
